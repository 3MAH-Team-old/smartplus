...
int main() {
    ...
    ident_control(n_param, n_consts, nfiles, ngen, aleaspace, apop, spop, ngboys, maxpop, probaMut, pertu, c, p0, lambdaLM, phiEps);
	...
	string data_exp_ext="dat";
    int sizev = 0;
    for(int i=0; i<nfiles;i++) {
		data_exp[i].import(data_exp_folder, data_exp_ext);
		data_weight[i].import(data_exp_folder, data_exp_ext);
    ...
    switch (TOOL) {
		case 1: { ///Use of the solver
            ...
            launch_solver(geninit, nfiles, params, consts, data_num_folder, data_num_name, data_num_ext);
			break;
		}		
/*		case 2: {///Use of the specific ODF code
	...
	for(int j=0; j<geninit.nindividuals; j++) {
		for(int i=0; i<nfiles; i++) {
			sstm << data_num_name << i+1 << "_" << j+1;
			data_num[i].name = sstm.str();
			sstm.str(std::string()); 		
			
			data_num[i].import(data_num_folder, data_num_ext);	
...
//	lambdaLM = gen[g].pop[0].cout*lambdaLM;
	...
	while((g<ngen)&&(gen[g].pop[0].cout > stationnarity)) {
...
/*			case 2: {
		...
		switch (TOOL) {
			case 1: {
				launch_solver(gboys_n, nfiles, params, consts, data_num_folder, data_num_name, data_num_ext);
				break;
			}		
/*			case 2: {
